    ...
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}${
          import.meta.env.VITE_ROTA_COLABORADORES
        }/tipo/${formattedRole}`
      );
            ...
            >
              ...
              <Grid item xs={5} sx={{ marginLeft: 2 }}>
                <Select
                  value={
                    selectedRoles[role] ||
                    hrJson[`month${monthIndex + 1}`][role.replace(' ', '_')] ||
                    ''
                  }
                  fullWidth
                  onChange={(event) =>
                    handleRoleChange(event, role, monthIndex + 1)
                  }
                  sx={{ height: 35 }}
                >
                  ...
                  {collaboratorsByRole[role] &&
                    collaboratorsByRole[role].map((colaborador, index) => (
