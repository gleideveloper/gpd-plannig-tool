...
const HrmPerMonthForm = ({
  ...
  const generateMonthLabels = () => {
    ...
    const templateData = newData.find(
      (item) => item.template_type === template_type
    );
  ...
  const fetchCurrentAllocationData = async () => {
    try {
      ...
      for (const { index, value } of resultado.map((value, index) => ({
        index,
        value,
      }))) {
  ...
  return (
    <>
      {selectedMonth ? (
        <HrmSpecificMonthModal
          monthLabel={selectedMonth}
          monthIndex={selectedMonthIndex}
          monthLabelInfo={monthLabels[selectedMonthIndex]}
          peakAmmountJson={peakAmmountJson}
          hrJson={hrJson}
                ...
                >
                  <Grid item xs={3} sx={{ paddingLeft: '24px' }}>
                    <span
                      style={
                        labelData.isSAMonth
                          ? { color: colorHighlight, fontWeight: 'bold' }
                          : {}
                      }
                    >
                      {labelData.label}
                  ...
                  <Grid item xs={3} sx={{ paddingLeft: '24px' }}>
                    <TextField
                      ...
                      fullWidth
                      sx={{
                        input: { cursor: 'default', textAlign: 'center' },
                        width: '80%',
                      }}
                      value={
                  ...
                  <Grid item xs={3} sx={{ paddingLeft: '24px' }}>
                    <Button
                      ...
                      sx={{ height: 40 }}
                      onClick={() =>
                        handleUpdateButtonClick(labelData.label, index)
                      }
                    >
                      Update
                    </Button>
